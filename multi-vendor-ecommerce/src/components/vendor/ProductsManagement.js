...
const ProductsManagement = () => {
    ...
    const fetchCategories = async () => {
        try {
            const response = await axios.get('{{base_url}}api/vendor/categories', {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const handleUpdateProduct = async () => {
        try {
            await axios.put(
                `{{base_url}}api/vendor/products/${editingProduct._id}`,
                { ...newProduct, category: newProduct.category },
                { headers: { Authorization: `Bearer ${token}` } }
            );
            fetchProducts();
    ...
    const handleDeleteProduct = async (productId) => {
        try {
            await axios.delete(`{{base_url}}api/vendor/products/${productId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchProducts();
        } catch (err) {
